...
func (f *FakeDbusClient) CallHandlerWithOutput(_ context.Context, _ Handler, _ ...interface{}) (string, error) {
	return "", f.CallError
}
...
var _ = Describe("HostServices", func() {
	...
	Describe("Handler Configuration", func() {
		DescribeTable("has the correct number of handlers",
			...
			Entry("default", "default", 7),
			Entry("edgecore", "edgecore", 7),
		)
		...
		DescribeTable("has the correct handler names in order",
			...
			Entry("default", "default", []string{"reboot", "save_config", "onie", "reboot_cause", "factory_reset", "reprovision", "systemd_restart"}),
			Entry("edgecore", "edgecore", []string{"reboot", "save_config", "onie", "reboot_cause", "factory_reset", "reprovision", "systemd_restart"}),
		)
	...
	Describe("Cross-Profile Differences", func() {
		...
		It("edgecore systemd_restart has explicit service_name arg, default has none", func() {
			...
			Expect(findHandler("default", "systemd_restart").Args).To(BeEmpty())
		})
	...
	Describe("DbusClient-dependent functions", func() {
		...
		Describe("HostServicesCompatibilityCheck", func() {
			...
			It("returns all handler names when all exist", func() {
				...
				Expect(compatible).To(ConsistOf("reboot", "save_config", "onie", "reboot_cause", "factory_reset", "reprovision", "systemd_restart"))
			})
		...
		Describe("InstallHostServiceModule", func() {
			It("returns error for unsupported module name", func() {
				...
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("not found in dbus mapping"))
			})
			...
			It("returns error for builtin handler not found on D-Bus", func() {
				// default/reboot is builtin with no module — cannot be installed
				err := InstallHostServiceModule(fakeClient, "default", "reboot")
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("has no module defined"))
			})
