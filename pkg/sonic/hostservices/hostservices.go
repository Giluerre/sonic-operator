...
type Handler struct {
	...
	// Module is the Python module name (e.g. "onie", "extension") that
	// provides this handler. Required when Builtin is false.
...
type DbusClient interface {
	...
	CallHandlerWithOutput(ctx context.Context, h Handler, args ...interface{}) (string, error)
	Close() error
}
...
func (c *SystemDbusClient) CallHandlerWithOutput(ctx context.Context, h Handler, args ...any) (string, error) {
	obj := c.conn.Object(h.ServiceName, dbus.ObjectPath(h.ObjectPath))
	call := obj.CallWithContext(ctx, h.Method, 0, args...)
	if call.Err != nil {
		return "", call.Err
...
func InstallHostServiceModule(client DbusClient, osProfile string, dbusModule string) error {
	...
	for _, h := range Modules[osProfile].Handlers {
		if h.Name == dbusModule {
			handler = &h
			break
		}
	}
	if handler == nil {
		return fmt.Errorf("handler %q not found in dbus mapping for profile %q", dbusModule, osProfile)
	}
	if handler.Module == "" {
		return fmt.Errorf("handler %q has no module defined", dbusModule)
	}

	pyFile := handler.Module + ".py"
	...
	if osProfile == "default" {
		...
		if !strings.Contains(contentStr, ", "+modName) {
			contentStr = strings.Replace(contentStr, "from host_modules import", "from host_modules import "+modName+", ", 1)
		}
		// Add registration if not present
		if !strings.Contains(contentStr, "'"+modName+"':") {
			configLine := "'config': config_engine.Config('config'),"
			className := strings.ToUpper(modName[:1]) + modName[1:]
			insertAfter := configLine + "\n        '" + modName + "': " + modName + "." + className + "('" + modName + "'),"
		...
		if !strings.Contains(sdContentStr, "'sonic-hostservice'") {
			sdContentStr = strings.Replace(sdContentStr, "ALLOWED_SERVICES = ['snmp',", "ALLOWED_SERVICES = ['sonic-hostservice','bgp', 'snmp',", 1)
