		...
		if err != nil {
			...
			go func() {
				for {
					...
					if !sa.dbusClient.ServiceAvailable("org.SONiC.HostService") {
						log.Printf("Waiting for sonic-hostservice (%s) to become available on D-Bus...", "org.SONiC.HostService")
						time.Sleep(15 * time.Second)
						continue
					}

					compatible, errors := hostservices.HostServicesCompatibilityCheck(sa.dbusClient, osProfile)
					if len(errors) > 0 {
...
func (m *SonicAgent) SaveConfig(ctx context.Context) *agent.Status {
	var handler *hostservices.Handler
	for _, h := range hostservices.Modules[m.OSProfile].Handlers {
		if h.Name == "save_config" {
			handler = &h
			break
		}
	}
	if handler == nil {
		return errors.NewErrorStatus(errors.BAD_REQUEST, fmt.Sprintf("no handler found for save_config in profile %s", m.OSProfile))
	}
...
func (m *SonicAgent) OnieBootModeInstall(ctx context.Context) *agent.Status {
	...
	if handler == nil {
		return errors.NewErrorStatus(errors.BAD_REQUEST, fmt.Sprintf("no handler found for onie in profile %s", m.OSProfile))
	}
...
func (m *SonicAgent) Reboot(ctx context.Context) *agent.Status {
	...
	if handler == nil {
		return errors.NewErrorStatus(errors.BAD_REQUEST, fmt.Sprintf("no handler found for reboot in profile %s", m.OSProfile))
	}
...
func RestartSystemdService(ctx context.Context, client hostservices.DbusClient, profile string, systemdServiceName string) *agent.Status {
	...
	for _, h := range hostservices.Modules[profile].Handlers {
		if h.Name == "systemd_restart" {
			handler = &h
			break
		}
	}
	if handler == nil {
		return errors.NewErrorStatus(errors.BAD_REQUEST, fmt.Sprintf("no handler found for restarting systemd service in profile %s", profile))
	}

	if err := client.CallHandler(ctx, *handler, systemdServiceName); err != nil {
		log.Printf("D-Bus call failed: %v", err)
		return errors.NewErrorStatus(errors.BAD_REQUEST, fmt.Sprintf("failed to restart systemd service %s via D-Bus: %v", handler.ServiceName, err))
	}
