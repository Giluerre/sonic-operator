...
type DbusClient interface {
	...
	ServiceAvailable(serviceName string) bool
...
func (c *SystemDbusClient) ServiceAvailable(serviceName string) bool {
	var hasOwner bool
	err := c.conn.BusObject().Call("org.freedesktop.DBus.NameHasOwner", 0, serviceName).Store(&hasOwner)
	if err != nil {
		return false
	}
	return hasOwner
...
func HostServicesCompatibilityCheck(client DbusClient, profile string) ([]string, []error) {
	...
	if !client.ServiceAvailable(firstHandler.ServiceName) {
